...
int main()
{
	...
	pHaarCascadepro = (CvHaarClassifierCascade*)cvLoad(pstrCascadeFileName);
	...
	{		
		...
		if(!pHaarCascade)
		{
			...
			CvSeq *pcvSeqproFaces = cvHaarDetectObjects(img, pHaarCascadepro, pcvMStorage1);
			dwTimeEnd = GetTickCount();
			printf("人脸个数: %d   识别用时: %d ms\n", pcvSeqproFaces->total, dwTimeEnd - dwTimeBegin);  
			for(int i = 0; i <pcvSeqproFaces->total; i++)
			{
				CvRect* r = (CvRect*)cvGetSeqElem(pcvSeqproFaces, i);	
				CvPoint center;
				int radius;
				center.x = cvRound((r->x + r->width * 0.5));
				center.y = cvRound((r->y + r->height * 0.5));
